...
p_abund <- ggplot(Total_abundance_df,
  ...
  labs(y = "Total plankton (segment corrected)", colour = "Water temperature (°C)") +
  scale_colour_viridis_c(option = "plasma", na.value = "grey70") +
...
p_effort <- ggplot(effort_df, aes(x = Year, y = n_segments)) +
...
SalpEuph.data <- Total_abundance_df %>%
  dplyr::select(all_of(c(
    colnames_metadata, Euphausid.IDs, Salp.IDs
  ))) %>%
  mutate(
...
AnnualBalance <- SalpEuph.data %>%
  group_by(Year) %>%
  summarise(SalpEuphIndex = mean(Salp.abundance, na.rm = TRUE) - mean(Euph.abundance, na.rm = TRUE))
...
SalpEuph <- ggplot(SalpEuph.data, aes(x = Year, y = Salp.abundance - Euph.abundance)) +
  ...
  scale_colour_manual(
    values = c(
      "FALSE" = "firebrick",
      # segment-level Euphausiid dominance
      "TRUE"  = "steelblue",
      # segment-level Salp dominance
  ...
  labs(y = "Salp – Euphausid abundance", x = "Year") +
  scale_x_continuous(breaks = year_breaks) +
  theme_classic() +
  theme(
    legend.position = "top",
