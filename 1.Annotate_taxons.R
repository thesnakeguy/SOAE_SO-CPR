...
parse_taxa_id <- function(id) {
  ...
  if (str_detect(id, "^[A-Z][a-z]+\\.[a-z]+\\.[a-z]+$") &
      parts[3] %in% life_stages) {
    return(list(
      ID = id,
      Genus = parts[1],
      Species = parts[2],
  ...
  if (str_detect(id, "\\.sp\\.\\.[a-z]+$") &
      (tolower(parts[length(parts)]) %in% life_stages)) {
  ...
  if (tolower(parts[length(parts)]) %in% life_stages &
      length(parts == 2)) {
    return(list(
      ID = id,
      HigherTaxon = parts[1],
  ...
  if (str_detect(id, "[A-Z][a-z]+\\.[a-z]+\\.indet$") &
      parts[2] %in% life_stages) {
    return(list(
      ID = id,
      HigherTaxon = parts[1],
      LifeStage = parts[2],
  ...
  if (str_detect(id, "^[A-Z][a-z]+\\.[a-z]+$") &
      (tolower(parts[length(parts)]) %ni%  life_stages)) {
    parts <- str_split(id, "\\.")[[1]]
    return(list(
      ID = id,
      Genus = parts[1],
...
process_taxon <- function(taxon) {
  ...
  tryCatch({
    ...
    message("Error in wm_name2id for taxon: ",
            taxon$Genus,
            " ",
            taxon$Species,
            " - ",
...
write.table(
  taxon_df_merged,
  "SO-CPR_ID_TaxonAnnotations.txt",
  quote = FALSE,
  sep = "\t"
)
